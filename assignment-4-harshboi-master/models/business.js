...
const { getPhotosByBusinessId, getImageInfoById } = require('./photo');
// const { getPhotosbyByBusinessId } = require('../models/photo');
...
async function getBusinessDetailsById(id) {
  ...
  if (business) {
    const photo_data = await getPhotosByBusinessId(id);
    var photo_edited = [];

    var i;
    for(i = 0; i < photo_data.length; i++){
        const insert = {
          businessid: photo_data[i].metadata.businessId,
          caption: photo_data[i].metadata.Caption,
          file: photo_data[i].filename,
          urls: `photos/media/photos/${photo_data[i].filename}`
        }
        photo_edited.push(insert);
    }
        business.photos = photo_edited;
  }
  return business;
}
